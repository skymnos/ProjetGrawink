...
{
	return "<rect x=\""+ std::to_string(m_origin.x) +"\" y=\""+ std::to_string(m_origin.y) +"\" width=\""+ std::to_string(m_width) +"\" height=\""+ std::to_string(m_height) +"\" stroke=\""+ m_stroke_color +"\" fill=\""+ m_fill_color +"\" stroke-width=\""+ std::to_string(m_stroke_width) +"\" transform = \"rotate("+ std::to_string(m_angle) +") translate("+ std::to_string(m_translate.x) + ","+  std::to_string(m_translate.y) + ") scale("+ std::to_string(m_scale) +") \"/>\n";
}
